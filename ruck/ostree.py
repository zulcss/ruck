...
class Ostree:
    ...
    def ostree_rollback(self, commit):
        ...
        return run_command(
            ["ostree", "reset", f"--repo={self.state.repo}", self.state.branch, commit],
            check=True,
        )

    def ostree_commit(
        self, root=None, repo=None, branch=None, subject=None, parent=None, msg=None
    ):
        """Commit rootfs to ostree repository."""
        cmd = ["ostree", "commit", "--skip-if-unchanged"]
        if repo:
            cmd += [f"--repo={repo}"]
        if subject:
    ...
    def open_ostree(self):
        ...
        if self.state.repo:
            ...
            if not repo.open(None):
                self.logging.error("Opening the archive OSTree repository failed.")
                sys.exit(1)
        else:
    ...
    def fetch(self, remote, branch, progress=None):
        ...
        try:
            repo.pull_with_options(
                remote, GLib.Variant("a{sv}", pull_options), progress, cancellable
            )
        except GLib.GError as e:
    ...
    def get_branch(self):
        ...
        if self.state.branch:
            ...
            try:
                ...
                # raise the exception
                if not e.matches(
                    GLib.KeyFile.error_quark(), GLib.KeyFileError.KEY_NOT_FOUND
                ):
                    raise (e)
    ...
    def remote_add(self):
        ...
        with self.console.status(f"Adding remote {self.state.remote}"):
            ...
            try:
                ...
                if check:
                    self.logging.info(f"Successfully added {self.state.remote}.")
            except GLib.GError as e:
    ...
    def remote_remove(self):
        ...
        try:
            ...
            if check:
                self.logging.info(f"Successfully removed {self.state.remote}.")
        except GLib.GError as e:
